 ...
 *
 * A signed-in launch shows several splashes back to back — the route
 * guard's, then the Suspense fallback while the screen downloads — and each
 ...
 * that turns up long after launch is simply the finished mark.
 */
...
/**
 * The short mark, written in the way it is on www: the outline is traced,
 * then the fill comes in behind the pen. If the wait outlasts the drawing,
 * the finished mark breathes, so the app still reads as alive.
 */
export function BrandMarkDraw({ className }: { className?: string }) {
  ...
  return (
    <svg
      ...
      className={cn('mark-draw text-volt shrink-0 overflow-visible', className)}
      role="img"
 ...
 *
 * A guard that knows where to go before the pen lifts holds the splash until
 * it does, so a fast launch shows a finished B rather than a stroke cut off
 * mid-letter. It never holds a page that showed no splash.
 */
